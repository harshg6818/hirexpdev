<template>
  <nav class="top-nav">
    <ul>
      <li
        v-for="(s,index) in sortByData"
        :key="index"
      >
        <div class="top-menu"
          @click="onClickSortBy(index)"
          :class="activeSort === index ? 'top-menu--active' : ''"
        >
          <div class="top-menu__title"> {{s}} </div>
        </div>
      </li>
    </ul>
  </nav>
...
export default {
  ...
  watch: {
    sortByData: {
      ...
      immediate: true,
      handler () {
        const active = Object.keys(this.sortByData)[0];
        this.activeSort = active;
        this.onClickSortBy(this.activeSort);
      }
    }
  },
