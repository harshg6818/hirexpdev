                  ...
                  <v-flex xs12 style="height:100%">
                    <v-layout v-if="table.stage && table.stage.length > 0" style="height:100%">
                      <v-flex xs4 class="list-sections">
                        <div v-for="(campaign, ci) in table.stage" class="pa-3 cursor-pointer"
                        v-if="$route.params.stageId !== campaign.id"
                        :class="activeCategory === ci ? 'active-campaign' : ''"
                        @click="getQuestions(campaign.id, ci)">
                          <strong> {{campaign.title}} </strong> <br/>
                          <span class="grey--text"> {{campaign.created_by_email}}  </span>
                          <!--<v-icon class="ml-3 grey--text"> fa fa-user </v-icon> {{campaign.created_by_email}} -->
                          <!--<span v-if="overallPartcipation(campaign)"> || {{overallPartcipation(campaign)}} % participation</span>-->
                          <span class="grey--text" v-if="campaign.participation_rate"> || {{campaign.participation_rate}} % participation</span>
                        </div>
                      </v-flex>
...
export default {
  ...
  methods: {
      ...
      axios.get(`${process.env.VUE_APP_ADHOC_API_URL}stage/list`, {
        ...
        if (response && response.data) {
          ...
          if (this.table.stage.length) {
            ...
            this.getQuestions(this.table.stage[0].id, 0);
          }
        }
      }, (response) => {
        this.config.initialLoading = false;
    ...
    getQuestions(campaignId, index) {
      ...
      axios.get(`${process.env.VUE_APP_ADHOC_API_URL}stage/questions`, {
        params: queryParams,
      }).then((response) => {
    ...
    updateQuestions(question) {
      ...
      if (match <= -1) {
        if (this.selectedQuestions[question.interaction_id]) {
          ...
          this.selectedQuestions[question.interaction_id] = true;
        }
      } else {
        this.allSelected = false;
        if (this.campaignQuestions && !this.selectedQuestions[question.interaction_id]) {
          delete this.selectedQuestions[question.interaction_id];
